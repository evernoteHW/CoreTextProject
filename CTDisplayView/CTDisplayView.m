...
- (void)userTapGestureDetected:(UIGestureRecognizer *)recognizer {
    ...
    if (_state == CTDisplayViewStateNormal) {
        ...
        CoreTextLinkData *linkData = [CTUtils touchLinkInView:self atPoint:point data:self.data];
        if (linkData) {
            NSLog(@"hint link!");
...
- (void)userLongPressedGuestureDetected:(UILongPressGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan ||
        ...
        CFIndex index = [CTUtils touchContentOffsetInView:self atPoint:point data:self.data];
...
- (void)drawAnchors {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if ([self isPosition:_selectionStartPosition inRange:range]) {
            ...
            _leftSelectionAnchor.frame = CGRectMake(origin.x,origin.y, _leftSelectionAnchor.frame.size.width, _leftSelectionAnchor.frame.size.height);
        }
        if ([self isPosition:_selectionEndPosition inRange:range]) {
            ...
            CGPoint origin = CGPointMake(linePoint.x + offset - 5, linePoint.y + ascent + 11);
            origin = CGPointApplyAffineTransform(origin, transform);
            _rightSelectionAnchor.frame = CGRectMake(origin.x,origin.y, _leftSelectionAnchor.frame.size.width, _leftSelectionAnchor.frame.size.height);
            break;
        }
    }
}
